...
namespace LowkeysDrops.API.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<UsuarioResponseDto> ActualizarUsuarioAsync(int idUsuario, ActualizarClienteDto dto)
        {
            var usuario = await _usuarioRepository.ObtenerPorIdAsync(idUsuario);
            if (usuario == null) throw new KeyNotFoundException("Usuario no encontrado.");
            ...
            if (existente != null && existente.IdUsuario != idUsuario)
                throw new Exception("CONFLICT:El correo ya está registrado por otro usuario.");
